...
export Arcfour *rc4init(int8 *key, int16 size) {
    ...
    if (!(p = malloc(sizeof(struct s_arcfour)))) assert_perror(errno);

    for (x=0;x<256;x++) p->s[x] = 0;  
    p->i= p->j= p->k = 0;
    temp1 = temp2 = 0;
    for (p->i = 0; p->i < 256; p->i++) p->s[p->i] = p->i;
...
export int8* rc4encrypt(Arcfour *p, int8 *cleartext, int16 size) {
    ...
    for (x=0; x<size; x++) ciphertext[x] = cleartext[x] ^ rc4byte(p);
    return ciphertext;
}
